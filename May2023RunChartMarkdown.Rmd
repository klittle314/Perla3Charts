...
measure_label <- "CO2e from activity X"
...
The three charts and explanations are based on the presentation in this paper:
...
- The time series graph for `r unit_label` A shows the monthly values summarized in the table on the previous slide.   
...
::: notes
1. The paper by Perla, Provost and Murray introduce four rules.  The trend and shift rules should only be applied to short series (n <= 20) as the probability of seeing a trend or shift signal is an increasing function of the length of the series.  For longer series, the number of runs rule will still apply.  Alternatively, you should consider constructing an appropriate control chart.  


2. We can show the graphical approach to finding the median on the graph from `r unit_name` A.  Grab a rectangle shape from the insert menu and stretch the top of the box until we get to between the 6th and 7th values in 2021.
:::
